...
int checkMiniBufferCubeAmmount(struct SetSlice setSlice) { int n; }

struct SetSlice checkSetSliceColor(struct SetSlice *setSlice) {
  if (strstr(setSlice->text, "red"))
    setSlice->color = RED;
  if (strstr(setSlice->text, "blue"))
    setSlice->color = BLUE;
  if (strstr(setSlice->text, "green"))
    setSlice->color = GREEN;
  return *setSlice;
}
